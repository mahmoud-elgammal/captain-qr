    ...
    for (i, color) in colors.iter().enumerate() {
        if *color == qrcode::Color::Dark {
            ...
            let draw_color = if let Some(grad_end) = config.gradient_color {
                ...
                let r = (config.fg_color.r as f32
                    + (grad_end.r as f32 - config.fg_color.r as f32) * progress)
                    as u8;
                let g = (config.fg_color.g as f32
                    + (grad_end.g as f32 - config.fg_color.g as f32) * progress)
                    as u8;
                let b = (config.fg_color.b as f32
                    + (grad_end.b as f32 - config.fg_color.b as f32) * progress)
                    as u8;
                Rgba([r, g, b, 255])
            } else {
